...
conf = json.load(open(r"C:\Users\POWER\Desktop\项目\sales_system\server_conf.json"))  # 加载配置信息
...
def goods_infor(barcode, pname, price, pnumber):
    '''
    函数功能：将商品信息写入数据库
    函数描述：
    barcode 条码
    ...
    try:
        with conn.cursor() as cur:  # 获取一个游标对象(Cursor类)，用于执行SQL语句
            # 执行任意支持的SQL语句
            cur.execute("insert into goods (barcode, pname, price, pnumber) values (%s, %s, %s, %s)", (barcode, pname, price, pnumber))
            r = cur.rowcount
            conn.commit()
    except:
        r = 0
